...
import { readableStreamToString, MultiwovenError, parseIfJson } from "./error";
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new MultiwovenError(e, parseIfJson(responseBody))
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new MultiwovenError(e, e.response?.data)
                }
                if (innerError instanceof MultiwovenError) {
                    // Got "MultiwovenError" from the above try block
                    throw innerError;
                }
